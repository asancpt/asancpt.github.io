...
Print <- function(cptid, DOI) {
    Ref <- GetBibEntryWithDOI(DOI)
    #cat(paste0("\n\n### ", paste0(strsplit(cptid, split = "-")[[1]][1:3], collapse = "-"), "\n- speaker: ", 
    #           strsplit(cptid, split = "-")[[1]][4], 
    #           ", _", Ref$title, "_ (", Ref$year, ") [Link](", Ref$url, ") "))
    cat(paste0("\n\n### ", cptid, "\n"))
    #cat(gsub("\\[1\\]", "", format(GetBibEntryWithDOI(DOI), style = "text")))
    print(Ref, .opts = list(bib.style = "authortitle", max.names = 1, no.print.fields = "url", style = "html"))
    cat('[<i class="fa fa-file-pdf-o"></i>PDF](')
    cat(paste0("ftp://172.21.61.202/cpt%20documents/BookJournal/2017/Journal/", cptid, "/", cptid, ".pdf)\n\n"))
